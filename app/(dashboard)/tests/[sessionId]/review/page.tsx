...
function ReviewContent() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // Prefer snapshot for frozen replay
        if (session.snapshot) {
          try {
            ...
            if (snap.questions?.length > 0) {
              if (!cancelled) {
                ...
                setConfidenceMap(snap.confidenceMap ?? {})
              }
              return
            }
          } catch { /* fall through */ }
        ...
        const reconstructedAnswers: Record<string, AnswerData> = {}
        ...
        if (!cancelled) {
          ...
          setConfidenceMap(reconstructedConfMap)
        }
      } catch (e) {
  ...
  const handleReportIssue = async () => {
    try {
      ...
      if (!user) return
      toast.promise(
        reportIssue({ user_id: user.$id, question_id: question.$id, mode: 'review' }),
        { loading: 'Reporting...', success: 'Issue reported!', error: 'Failed to report' }
      )
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[80vh] gap-4">
        ...
        <p className="text-sm text-gray-400 font-medium">Loading review...</p>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-[#F8F9FC]">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8 grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8">
        ...
        <div className="space-y-6">
          ...
          {/* Subtopic */}
          {(question.subtopic || question.tags?.[0]) && (
