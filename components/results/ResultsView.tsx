...
import {
  ...
  AlertCircle, Lightbulb, Brain, Target, Zap, Loader2, ArrowLeft
} from 'lucide-react'
...
interface ResultsViewProps {
  sessionId: string
  // When true: shows "Back to My Tests" instead of Dashboard, no Retake button
  replayMode?: boolean
}
  ...
  useEffect(() => {
    const rehydrate = async () => {
      ...
      try {
        ...
        // Prefer snapshot for exact replay (questions don't change even if DB changes)
        if (session.snapshot) {
          try {
            ...
            console.warn('Failed to parse snapshot, falling back to DB:', e.message)
          }
        }

        // Fallback: fetch from Appwrite attempts + questions
    ...
    rehydrate()
  }, [sessionId, questions.length, setQuestions, setAnswers, setConfidenceForQuestion, setTestMode, setPaperLabel, setElapsed, setConfidenceMap])

  // ─── ANALYTICS CALCULATION ──────────────────────────────────
  const generatedAnalytics = useMemo(() =>
    generateTestAnalytics({
      questions,
      attempts: Object.entries(answers).map(([id, ans]: [string, any]) => {
    ...
    <div className="min-h-screen bg-[#F8F9FC] pb-24">
      <div className="sticky top-0 z-30 bg-white/70 backdrop-blur-xl border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            {replayMode ? (
              <button onClick={() => router.push('/tests')} className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-gray-500 hover:text-gray-900 transition-colors">
                <ArrowLeft className="h-4 w-4" /> My Tests
              </button>
            ) : (
              <button onClick={() => router.push('/dashboard')} className="hidden md:flex items-center gap-2 px-4 py-2 text-sm font-bold text-gray-500 hover:text-gray-900 transition-colors">
                <Home className="h-4 w-4" /> Dashboard
              </button>
            )}
            {!replayMode && (
              <button onClick={() => { reset(); router.push('/quiz') }} className="bg-white border border-gray-200 px-5 py-2.5 rounded-2xl text-sm font-black text-gray-900 hover:bg-gray-50 transition-all flex items-center gap-2 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-[#111111] rounded-[2rem] overflow-hidden flex flex-col">
            <div className="p-7 flex-1">
              <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-3">{paperLabel || 'Mock Analysis'} · {questions.length} Qs</p>
              ...
              <div className="mt-5">
                ...
                <div className="h-1 bg-gray-800 rounded-full overflow-hidden">
                  <div className="h-full bg-[#00E5BE] rounded-full transition-all duration-1000" style={{ width: `${(Object.keys(answers).length / questions.length) * 100}%` }} />
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-white/5">
              ...
              <button onClick={() => handleQuestionClick(0)} className="bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 transition-colors">
            ...
            <div className="flex-1 min-h-[280px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={analytics.subjectStats} layout="vertical" margin={{ top: 0, right: 30, left: 20, bottom: 0 }}>
                  ...
                  <Tooltip cursor={{ fill: 'rgba(0,0,0,0.02)' }} contentStyle={{ backgroundColor: '#FFF', border: '1px solid #F1F5F9', borderRadius: '12px', fontSize: '10px', fontWeight: 'bold' }} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white rounded-[2rem] border border-emerald-100 shadow-sm p-8 flex flex-col">
            <div className="flex items-center justify-between mb-6">
              <div className="w-12 h-12 rounded-2xl bg-emerald-50 flex items-center justify-center"><CheckCircle className="h-6 w-6 text-emerald-600" /></div>
            ...
            <TaggedQuestionsDropdown tag="sure" title="Review Sure Items" questions={questions} answers={answers} confidenceMap={confidenceMap} onQuestionClick={handleQuestionClick} />
          </div>

          <div className="bg-white rounded-[2rem] border border-blue-100 shadow-sm p-8 flex flex-col">
            <div className="flex items-center justify-between mb-6">
              <div className="w-12 h-12 rounded-2xl bg-blue-50 flex items-center justify-center"><Brain className="h-6 w-6 text-blue-600" /></div>
            ...
            <TaggedQuestionsDropdown tag="fifty_fifty" title="Review Fifty-Fifty Items" questions={questions} answers={answers} confidenceMap={confidenceMap} onQuestionClick={handleQuestionClick} />
          </div>

          <div className="bg-white rounded-[2rem] border border-orange-100 shadow-sm p-8 flex flex-col">
            <div className="flex items-center justify-between mb-6">
              <div className="w-12 h-12 rounded-2xl bg-orange-50 flex items-center justify-center"><Zap className="h-6 w-6 text-orange-600" /></div>
            ...
            <TaggedQuestionsDropdown tag="guess" title="Review Guess Items" questions={questions} answers={answers} confidenceMap={confidenceMap} onQuestionClick={handleQuestionClick} />
          </div>
        </div>
        ...
        <div className="pt-8 border-t border-gray-100">
          ...
          {(() => {
            const mergedAnswers = Object.fromEntries(
              Object.entries(answers).map(([qId, ans]: [string, any]) => [
                qId,
        ...
        {analytics.suggestions.length > 0 && (
          <div className="bg-gray-900 rounded-[3rem] p-10 text-white relative overflow-hidden">
            ...
            <div className="relative z-10">
              <h3 className="text-xl font-black uppercase tracking-widest mb-6 flex items-center gap-3"><Zap className="h-6 w-6 text-[#FF6B00]" /> Strategy Protocol</h3>
